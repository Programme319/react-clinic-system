...
import { fetchStaffList, setStaffActive } from '@/lib/adminApi';
...
export default function StaffManagement() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const list = await fetchStaffList(authUser.id);
      setStaff(list);
  ...
  const toggleActive = async (id, active) => {
    setError('');
    try {
      await setStaffActive(authUser.id, id, !active);
      ...
      <div className="panel">
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>
              {staff.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--color-text-muted)' }}>No staff members found.</td></tr>
              ) : staff.map((u) => (
                <tr key={u.id}>
                  <td>{u.name}</td>
                  <td>{u.email}</td>
